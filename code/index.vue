  ...
  <div :class="$style.box">
    ...
    >
      <div :class="$style.bd">
        ...
        />
        <img :class="$style.bg" :src="item.coverImage" />
        <div :class="$style.wrap">
          <img
            :class="$style.riverdinwei"
            src="https://img.alicdn.com/tfs/TB1mtZRoVT7gK0jSZFpXXaTkpXa-28-36.png"
          />
          <span :class="$style.distance">距离500m</span>
        </div>
      </div>
      ...
      <div :class="$style.ft">
        <div :class="$style.block">
          ...
          />
          <span :class="$style.fashionHome">{{ item.user.userName }}</span>
        </div>
        <div v-if="isReadCountShow(item.readCount)" :class="$style.group">
          <img
            :class="$style.favorite"
            src="https://img.alicdn.com/tfs/TB1arwYo7T2gK0jSZFkXXcIQFXa-46-44.png"
          />
          <span :class="$style.num">{{ item.readCount }}</span>
        </div>
      </div>
    </div>
  </div>
...
<style module lang="scss" src="./index.scss" />
